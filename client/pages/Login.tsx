...
//         setResponse('Login successful! Redirecting to dashboard...');
        
//         if (onLoginSuccess) {
//           onLoginSuccess(result.user_info);
//         }
...
//   // Handle Google login (placeholder for now)
//   const handleGoogleLogin = () => {
//     setResponse('Google authentication is not yet configured. Please use email login for now.');
//   };
...
//     setResponse('Password reset functionality is not yet implemented. Please contact support.');
//   };
...
//             type="button"
//             className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-300 text-gray-700 hover:bg-gray-50 py-3 rounded-xl font-semibold transition-all duration-200"
//             disabled={isLoading}
//           >
  ...
  const handleOAuthLogin = async (provider: string) => {
    ...
    try {
      ...
      if (result.success && result.user_info) {
        ...
        setResponse(result.message || `${provider} login failed.`);
        setShowSignupCTA(false);
      }
    } catch (error) {
